...
Connecting through Ethernet is done by plugging in the cable and configuring
your computer to be on the `192.168.255.0/24` network (using the address
`192.168.255.1` is not allowed). Then you can navigate to the `192.168.255.1`
address in your web browser to access the Web UI.

Connecting through WiFi is done by connecting to the device’s Access Point. The
SSID is in the following format: `hardwario-gauger-IDIDIDIDIDID`. The default
password is `12345678`. After connecting to the WiFi network, you will
automatically be assigned an IP address. Then you can access the device at
`192.168.254.1`.
